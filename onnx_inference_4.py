...
def main():
    parser = argparse.ArgumentParser(description="ONNX 4-models tracking with interactive box selection")
    parser.add_argument("--template_encoder_path", type=str, default="out_4/template_encoder.onnx", help="Path to Template Encoder ONNX model")
    parser.add_argument("--online_template_encoder_path", type=str, default="out_4/online_template_encoder.onnx", help="Path to Online Template Encoder ONNX model")
    parser.add_argument("--search_encoder_path", type=str, default="out_4/search_encoder.onnx", help="Path to Search Encoder ONNX model")
    parser.add_argument("--tracking_head_path", type=str, default="out_4/tracking_head.onnx", help="Path to Tracking Head ONNX model")
    ...
    template_encoder = onnxruntime.InferenceSession(args.template_encoder_path)
    online_template_encoder = onnxruntime.InferenceSession(args.online_template_encoder_path)
    search_encoder = onnxruntime.InferenceSession(args.search_encoder_path)
    tracking_head = onnxruntime.InferenceSession(args.tracking_head_path)
    
    reg_tokens_path = os.path.join(os.path.dirname(args.template_encoder_path), "reg_tokens.npy")
    if os.path.exists(reg_tokens_path):
        reg_tokens_init = np.load(reg_tokens_path)
        print(f"Loaded reg_tokens from {reg_tokens_path}, shape: {reg_tokens_init.shape}")
    else:
        print(f"Warning: reg_tokens file not found at {reg_tokens_path}, using random initialization")
        reg_tokens_init = np.random.randn(1, 4, embed_dim).astype(np.float32)
    ...
    while True:
        ...
        if tracking_started and template_feat is not None and online_template_feat is not None:
            ...
            ort_inputs_search = {
                ...
                "template_feat": template_feat.astype(np.float32),
                "online_template_feat": online_template_feat.astype(np.float32),
                "reg_tokens": reg_tokens.astype(np.float32),
            }
            
            search_outs = search_encoder.run(None, ort_inputs_search)
            search_feat = search_outs[0]
            reg_tokens = search_outs[1]
            
            ort_inputs_head = {
                "reg_tokens": reg_tokens.astype(np.float32),
            }
            
            head_outs = tracking_head.run(None, ort_inputs_head)
            pred_boxes = head_outs[0]
            pred_scores = head_outs[1] if len(head_outs) > 1 else None
            ...
            score = pred_scores[0, 0] if pred_scores is not None and pred_scores.ndim > 1 else (pred_scores[0] if pred_scores is not None else 1.0)
        ...
        if first_frame_paused:
            while first_frame_paused:
                ...
                if init_box is not None:
                    ...
                    template_outs = template_encoder.run(None, {"template": template_input.astype(np.float32)})
                    template_feat = template_outs[0]
                    
                    online_template_outs = online_template_encoder.run(None, {"online_template": template_input.astype(np.float32)})
                    online_template_feat = online_template_outs[0]
                    
                    reg_tokens = reg_tokens_init.copy()
